...
const validarCampos = (sucursal) => {
  const campos = ["nombre", "direccion", "telefono_Contacto", "nombre_Encargado"];
  return campos.every((campo) => sucursal[campo]?.trim() !== "");
};


  const handleAddSucursal = async (e) => {
    e.preventDefault();
    if (!validarCampos(newSucursal)) {
      closeAddModal();
      mostrarAlerta("error", "Campos vacíos", "Por favor, completa todos los campos sin dejar espacios en blanco.");
      return;
    }
    try {
      const response = await fetch(`${API_URL}`, {
